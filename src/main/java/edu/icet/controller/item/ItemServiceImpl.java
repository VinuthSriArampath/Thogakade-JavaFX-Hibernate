...
public class ItemServiceImpl implements ItemService{
    ...
    public Boolean addItem(Item item){
        ...
        try {
            boolean b = CrudUtil.execute(sql,item.getItemcode(),item.getDesc(),item.getPacksize(),item.getUnitprize(),item.getQty());
            if (b){
                return true;
            }
            return false;
    ...
    public Boolean updateItem(Item item) {
        ...
        try {
            if (CrudUtil.execute(sql,item.getDesc(),item.getPacksize(),item.getUnitprize(),item.getQty(),item.getItemcode())){
                return true;
            }
            return false;
        } catch (SQLException e) {
